...
import { fillBlankEnApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
...
export default function FillBlankEnGroupsPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await fillBlankEnApi.getGroups()
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await fillBlankEnApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await fillBlankEnApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await fillBlankEnApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/fill-blank-en/${groupId}`)
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Zarządzaj ćwiczeniami z lukami
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/fill-blank-en'}
                        className="gap-2"
                    >

...
import { fillBlankApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Plus, Pencil, Trash2, Loader2, AlertCircle, ArrowRight, TextCursorInput, GraduationCap } from 'lucide-react'
...
export default function FillBlankGroupsPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await fillBlankApi.getGroups(activeLanguage)
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await fillBlankApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await fillBlankApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await fillBlankApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/fill-blank/${groupId}`)
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-[50vh]">
                <div className="text-center space-y-4">
                    ...
                    <Button onClick={loadGroups}>Sprobuj ponownie</Button>
                </div>
            </div>
        )
    }
        ...
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Wybierz grupe cwiczen do edycji
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/fill-blank'}
                        className="gap-2"
                    >
                        <GraduationCap className="w-5 h-5" /> Tryb Nauki
                    </Button>
                    <Button onClick={openCreateDialog} size="lg" className="gap-2">
                        <Plus className="w-5 h-5" /> Dodaj grupe
                    </Button>
                </div>
            </div>
            ...
            {groups.length === 0 ? (
                <div className="text-center p-12 bg-gray-50 rounded-xl border-2 border-dashed">
                    <p className="text-muted-foreground">Brak grup. Dodaj pierwsza grupe.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {groups.map((group) => (
                        <Card
                            key={group.id}
                            className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-emerald-200"
                            onClick={() => handleGroupClick(group.id)}
                        >
                            <CardHeader>
                                <div className="flex items-start justify-between">
                                    <div className="bg-emerald-100 p-2 rounded-lg">
                                        <TextCursorInput className="w-6 h-6 text-emerald-600" />
                                    </div>

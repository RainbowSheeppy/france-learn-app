...
import { groupsEnApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Plus, Pencil, Trash2, Loader2, AlertCircle, ArrowRight, BookOpen, GraduationCap } from 'lucide-react'
import GroupDialog from '@/components/admin/GroupDialog'
import DeleteGroupDialog from '@/components/admin/DeleteGroupDialog'

export default function GroupsEnPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await groupsEnApi.getAll()
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await groupsEnApi.create(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await groupsEnApi.update(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await groupsEnApi.delete(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/groups-en/${groupId}`)
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Zarządzaj grupami fiszek angielskich
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/fiszki-en'}
                        className="gap-2"
                    >

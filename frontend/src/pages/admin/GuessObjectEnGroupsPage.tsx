...
import { guessObjectEnApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
...
export default function GuessObjectEnGroupsPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await guessObjectEnApi.getGroups()
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await guessObjectEnApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await guessObjectEnApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await guessObjectEnApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/guess-object-en/${groupId}`)
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-amber-600 to-yellow-600 bg-clip-text text-transparent">
                        Zgadnij Przedmiot (EN)
                    </h1>
                    <p className="text-muted-foreground mt-2">
                        Zarządzaj grupami zagadek słownych
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/guess-object-en'}
                        className="gap-2"
                    >

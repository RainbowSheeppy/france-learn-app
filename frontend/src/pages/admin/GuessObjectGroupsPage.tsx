...
import { guessObjectApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
...
export default function GuessObjectGroupsPage() {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await guessObjectApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await guessObjectApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await guessObjectApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/guess-object/${groupId}`)
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Wybierz grupe zagadek do edycji
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/guess-object'}
                        className="gap-2"
                    >
            ...
            {groups.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {groups.map((group) => (
                        <Card
                            key={group.id}
                            className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-amber-200"

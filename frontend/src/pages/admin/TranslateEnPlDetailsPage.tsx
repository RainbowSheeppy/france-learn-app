...
import { translateEnPlApi, aiEnApi, type Group, type TranslateEnItem, type TranslateEnItemCreate, type TranslateEnItemUpdate, type GeneratedItemEn } from '@/lib/api'
...
export default function TranslateEnPlDetailsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            setLoading(true)
            const groups = await translateEnPlApi.getGroups()
            const foundGroup = groups.find(g => g.id === groupId)
            if (!foundGroup) throw new Error("Group not found")
            setGroup(foundGroup)

            const itemsData = await translateEnPlApi.getAllItems(groupId)
            setItems(itemsData)
        } catch (err: any) {
    ...
    const handleCreateItem = async (data: any) => {
        // For EN->PL: text_pl contains Polish answer, text_en contains English question
        const createData: TranslateEnItemCreate = {
            text_en: data.text_pl, // Source (English) - mapped from first field
            text_pl: data.text_fr, // Target (Polish) - mapped from second field
            group_id: groupId
        }
        await translateEnPlApi.createItem(createData)
        const itemsData = await translateEnPlApi.getAllItems(groupId!)
        setItems(itemsData)
    }
    ...
    const handleEditItem = async (data: any) => {
        ...
        const updateData: TranslateEnItemUpdate = {
            text_en: data.text_pl,
            text_pl: data.text_fr
        }
        await translateEnPlApi.updateItem(editingItem.id, updateData)
        const itemsData = await translateEnPlApi.getAllItems(groupId!)
        setItems(itemsData)
    }
    ...
    const handleDeleteItem = async () => {
        ...
        try {
            await translateEnPlApi.deleteItem(deletingItem.id)
            const itemsData = await translateEnPlApi.getAllItems(groupId!)
            setItems(itemsData)
        } finally {
    ...
    const handleSaveGenerated = async () => {
        ...
        setSavingGen(true)
        try {
            // For EN->PL, we swap the generated content: EN becomes source, PL becomes target
            await translateEnPlApi.batchCreate({
                group_id: groupId,
                items: generatedItems.map(item => ({
                    text_en: item.text_en, // English as source
                    text_pl: item.text_pl, // Polish as target
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="space-y-4">
                <Button variant="ghost" onClick={() => navigate('/admin/translate-en-pl')} className="pl-0 hover:pl-2 transition-all">
                    ...
                    <Card key={item.id} className="group hover:border-teal-200 transition-all">
                        <CardContent className="flex justify-between items-center p-4">
                            <div className="flex-1">
                                <p className="font-semibold text-lg">{item.text_en}</p>
                                <p className="text-sm text-muted-foreground">EN (Pytanie)</p>
                            </div>
                            <ArrowRight className="text-muted-foreground mx-4" />
                            <div className="flex-1 text-right">
                                <p className="font-semibold text-lg text-teal-600">{item.text_pl}</p>
                                <p className="text-sm text-muted-foreground">PL (Odpowiedź)</p>
                            </div>
                            {item.category && (
                                <div className="mx-4 px-2 py-1 bg-teal-100 text-teal-700 text-xs rounded-full">
                                    {item.category}
                                </div>
                            )}
            ...
            <TranslateItemDialog
                ...
                item={editingItem ? { ...editingItem, text_pl: editingItem.text_en, text_fr: editingItem.text_pl } as any : null}
                mode={dialogMode}
                labels={{ pl: 'Tekst angielski (Pytanie)', fr: 'Tekst polski (Odpowiedź)' }}
            />
            ...
            <Dialog open={genModalOpen} onOpenChange={setGenModalOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2"><Wand2 className="w-5 h-5 text-teal-600" /> Generuj Zdania AI (Angielski → Polski)</DialogTitle>
                        <DialogDescription>

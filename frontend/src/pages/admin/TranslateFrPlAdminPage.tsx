...
export default function TranslateFrPlAdminPage() {
    ...
    const loadData = async () => {
        try {
            setLoading(true)
            const groupsData = await translateFrPlApi.getGroups()
    ...
    const loadItems = async (groupId: string) => {
        try {
            const data = await translateFrPlApi.getAllItems(groupId)
            setItems(data)
        } catch (err) {
            console.error(err)
        }
    ...
    const handleCreateGroup = async () => {
        try {
            await translateFrPlApi.createGroup({ name: newGroupName, description: newGroupDesc })
            await loadData()
    ...
    const handleImport = async () => {
        ...
        setImporting(true)
        try {
            await translateFrPlApi.importFromCsv(importGroup, importFile)
            setImportOpen(false)
            setImportFile(null)
        ...
        try {
            await translateFrPlApi.batchCreate({
                ...
                items: generatedItems.map(item => ({
                    text_fr: item.text_fr, // Question
                    text_pl: item.text_pl  // Answer
                }))
            })
            ...
            <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" className="border-pink-200 hover:bg-pink-50 text-pink-700" onClick={() => { setGenGroup(selectedGroup === 'all' ? '' : selectedGroup); setGenModalOpen(true); }}>
                            <Wand2 className="w-4 h-4 mr-2" /> Generate Ai
                        </Button>
                        <Button onClick={() => window.location.href = '/learn/translate-fr-pl'}>
                ...
                <Dialog open={importOpen} onOpenChange={setImportOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Wybierz plik CSV z kolumną 'text_pl'. System przetłumaczy na FR i ustawi FR jako pytanie.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                            <div className="space-y-2">
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" className="border-pink-200 hover:bg-pink-50 text-pink-700" onClick={() => { setGenGroup(selectedGroup); setGenModalOpen(true); }}>
                        <Wand2 className="w-4 h-4 mr-2" /> Generate Ai
                    </Button>
                    ...
                    <Card key={item.id}>
                        <CardContent className="flex justify-between items-center p-4">
                            ...
                            <div className="flex-1 text-right">
                                <p className="font-semibold text-lg text-pink-600">{item.text_pl}</p>

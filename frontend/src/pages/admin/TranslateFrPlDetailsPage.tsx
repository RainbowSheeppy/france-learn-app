...
import { translateFrPlApi, aiApi, type Group, type TranslateItem, type TranslateItemCreate, type TranslateItemUpdate, type GeneratedItem } from '@/lib/api'
...
export default function TranslateFrPlDetailsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            setLoading(true)
            const groups = await translateFrPlApi.getGroups()
            const foundGroup = groups.find(g => g.id === groupId)
            if (!foundGroup) throw new Error("Group not found")
            setGroup(foundGroup)

            const itemsData = await translateFrPlApi.getAllItems(groupId)
            setItems(itemsData)
        } catch (err: any) {
    ...
    const handleCreateItem = async (data: TranslateItemCreate) => {
        await translateFrPlApi.createItem({ ...data, group_id: groupId })
        const itemsData = await translateFrPlApi.getAllItems(groupId!)
        setItems(itemsData)
    }
    ...
    const handleEditItem = async (data: TranslateItemUpdate) => {
        ...
        await translateFrPlApi.updateItem(editingItem.id, data)
        const itemsData = await translateFrPlApi.getAllItems(groupId!)
        setItems(itemsData)
    }
    ...
    const handleDeleteItem = async () => {
        ...
        try {
            await translateFrPlApi.deleteItem(deletingItem.id)
            const itemsData = await translateFrPlApi.getAllItems(groupId!)
            setItems(itemsData)
        } finally {
    ...
    const handleImport = async () => {
        ...
        setImporting(true)
        try {
            await translateFrPlApi.importFromCsv(groupId, importFile)
            setImportOpen(false)
            setImportFile(null)
            const itemsData = await translateFrPlApi.getAllItems(groupId)
            setItems(itemsData)
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="space-y-4">
                <Button variant="ghost" onClick={() => navigate('/admin/translate-fr-pl')} className="pl-0 hover:pl-2 transition-all">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" className="border-pink-200 hover:bg-pink-50 text-pink-700" onClick={() => setGenModalOpen(true)}>
            ...
            <TranslateItemDialog
                ...
                item={editingItem}
                mode={dialogMode}
                labels={{ pl: 'Tekst polski (Odpowiedź)', fr: 'Tekst francuski (Pytanie)' }}
            />

...
import { translateFrPlApi, aiApi, type Group, type GroupCreate, type GroupUpdate, type TranslateItemCreate } from '@/lib/api'
...
export default function TranslateFrPlGroupsPage() {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await translateFrPlApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await translateFrPlApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await translateFrPlApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/translate-fr-pl/${groupId}`)
    }

    const handleGenerate = async (level: string, count: number, topic: string) => {
        setGenLoading(true)
        try {
            // 1. Generate items using AI (Note: reuse API but maybe mapped differently? No, api.generate returns text_pl and text_target, which is fine)
            // But we need to be careful about direction. 
            // In TranslateFrPl: Target is FR (from which we translate), PL is answer.
            // aiApi.generate returns GeneratedItem: { text_pl, text_target }
            // For FR->PL: text_fr should be Question, text_pl should be Answer.
            // But generated item has text_target (FR) and text_pl (PL).
            ...
            const newGroup = await translateFrPlApi.createGroup({
                name: groupName,
            ...
            const itemsToCreate: TranslateItemCreate[] = generatedItems.map(item => ({
                text_target: item.text_target, // FR (Question)
                text_pl: item.text_pl,         // PL (Answer)
                category: item.category || 'mixed',
                group_id: newGroup.id
            }))

            await translateFrPlApi.batchCreate({
                items: itemsToCreate,
                group_id: newGroup.id
            })
        ...
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/translate-fr-pl'}
                        className="gap-2"
                    >
            ...
            {groups.length === 0 ? (
                <div className="text-center p-12 bg-gray-50 rounded-xl border-2 border-dashed">
                    ...
                    <div className="mt-4 flex justify-center gap-4">
                        ...
                        <Button onClick={() => setGenDialogOpen(true)} variant="outline" className="border-pink-200 text-pink-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {groups.map((group) => (
                        <Card
                            key={group.id}
                            className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-pink-200"
                            onClick={() => handleGroupClick(group.id)}
                        >
                            <CardHeader>
                                <div className="flex items-start justify-between">
                                    <div className="bg-pink-100 p-2 rounded-lg">
                                        <BookA className="w-6 h-6 text-pink-600" />
                                    </div>
                                    <ArrowRight className="w-5 h-5 text-muted-foreground group-hover:text-pink-600 group-hover:translate-x-1 transition-all" />
                                </div>

...
import { translatePlEnApi, type Group, type GroupCreate, type GroupUpdate } from '@/lib/api'
...
export default function TranslatePlEnGroupsPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await translatePlEnApi.getGroups()
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await translatePlEnApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await translatePlEnApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await translatePlEnApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
        } finally {
    ...
    const handleGroupClick = (groupId: string) => {
        navigate(`/admin/translate-pl-en/${groupId}`)
    }
        ...
        <div className="max-w-6xl mx-auto space-y-6 animate-fade-in px-6 py-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/translate-pl-en'}
                        className="gap-2"
                    >
                            ...
                            <CardHeader>
                                <div className="flex items-start justify-between">
                                    <div className="bg-purple-100 p-2 rounded-lg">
                                        <BookA className="w-6 h-6 text-purple-600" />
                                    </div>

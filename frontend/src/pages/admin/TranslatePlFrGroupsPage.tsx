...
export default function TranslatePlFrGroupsPage() {
    ...
    const loadGroups = async () => {
        try {
            setLoading(true)
            setError('')
            const data = await translatePlFrApi.getGroups(activeLanguage)
            setGroups(data)
        } catch (err: any) {
    ...
    const handleCreate = async (data: GroupCreate) => {
        await translatePlFrApi.createGroup(data)
        await loadGroups()
    }

    const handleEdit = async (data: GroupUpdate) => {
        if (!editingGroup) return
        await translatePlFrApi.updateGroup(editingGroup.id, data)
        await loadGroups()
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            await translatePlFrApi.deleteGroup(deletingGroup.id)
            await loadGroups()
        } catch (err) {
            console.error(err)
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
                        Tłumaczenie {getTranslateFromPLLabel(activeLanguage)}
                    </h1>
                ...
                <div className="flex gap-2">
                    <Button
                        variant="secondary"
                        onClick={() => window.location.href = '/learn/translate-pl-fr'}
                        className="gap-2"
                    >
